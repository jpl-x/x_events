...
void TrackManager::manageTracks(MatchList &matches,
  ...
  lost_slam_idxs_.clear();
  while (it < slam_trks_.size()) {
    bool isTrackLost = true;
    unsigned int m = 0;
    while (m < matches.size()) {
      // Check if the last feature of the persistent track is the previous match
      if (slam_trks_[it].back().getX() == matches[m].previous.getX()
          && slam_trks_[it].back().getY() == matches[m].previous.getY()) {
        ...
        bin_track_idx[bin_nbr].push_back(it);
        bin_track_idx_per[bin_nbr].push_back(it + lost_per_count);
        ...
        slam_trks_[it].push_back(matches[m].current);
    ...
    if (isTrackLost) {
      ...
      continue; // continue to avoid incrementing track iterator it
    }
    it++; // increment the track iterator
  }
  ...
  while (m < matches.size()) // Matches are in descending FAST order
  {
    it = 0;
    bool isTrackLost = true;
    while (it < previous_opp_trks.size()) {
      // Check if the last feature of the opportunistic track is the previous match
      if (previous_opp_trks[it].back().getX() == matches[m].previous.getX()
          && previous_opp_trks[it].back().getY() == matches[m].previous.getY()) {
        isTrackLost = false; // we found the track, so it is not lost
        // Add the matched feature to the track
        previous_opp_trks[it].push_back(matches[m].current);
        break; // leave the while loop on the opportunistic tracks
      }
      it++; // increment the opportunistic track iterator
    }

    if (!isTrackLost) // if the track is not lost
    {
      // Push it to the opportunistic tracks
      opp_trks_.push_back(previous_opp_trks[it]);
      // Remove from the previous opportunistic list
      previous_opp_trks.erase(previous_opp_trks.begin() + it);
  ...
  it = 0;
  while (it < opp_trks_.size()) // Tracks are sorted by length, longest to shortest
  {
    // Find and store current bin for this feature
    Feature &feature = opp_trks_[it].back();
    img.setTileForFeature(feature);
    const unsigned int bin_nbr =
      feature.getTileRow() * img.getNTilesW() + feature.getTileCol();

    // If track is long enough (quality metric)
    if (opp_trks_[it].size() > min_track_length - 1) {
      ...
      if ((slam_trks_.size() + new_slam_trks_.size()) < n_slam_features_max) {
        ...
        new_slam_trks_.push_back(opp_trks_[it]);
        ...
        // spread out their distribution
      {
        ...
        if (bin_track_idx[idx_bin_max_per].size() >
          ...
          if (opp_trks_[it].size() > n_poses_max - 1) {
            ...
            it++; // increment the match iterator
        }
      }
    } else
      it++; // increment the match iterator
  ...
  for (const auto& trk : new_slam_trks_) {
...
void TrackManager::plotFeatures(TiledImage &img,
  ...
  int baseline = 0;
  cv::Size textSize = cv::getTextSize(slam_str, cv::FONT_HERSHEY_PLAIN, scale, 1.5, &baseline);
  int offset = textSize.width;
  cv::putText(img, oppStr, cv::Point((int) 10 + offset, (int) camera_.getHeight() - 10), cv::FONT_HERSHEY_PLAIN,
              scale, orange, 1.5, 8, false);
  textSize = cv::getTextSize(oppStr, cv::FONT_HERSHEY_PLAIN, scale, 1.5, &baseline);
