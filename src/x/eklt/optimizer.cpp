...
namespace eklt {
  ...
  void Optimizer::setParams(const x::EkltParams &params) {
    params_ = params;
  }
  ...
  void Optimizer::precomputeLogImageArray(const eklt::Patches &patches, const eklt::ImageBuffer::iterator &image_it) {
    cv::Mat I_x, I_y;
  ...
  void Optimizer::optimizeParameters(const cv::Mat &event_frame, eklt::Patch &patch) {
    double norm = 0;
